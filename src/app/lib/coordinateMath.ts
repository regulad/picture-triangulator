...
export function latLonToGPSPointData(
  latLon: LatLon,
...
export function distanceBetweenPointsMeters(
  point1: GPSPointData,
  point2: GPSPointData,
): number {
...
export function triangulatePointFromPointData(
  point1: GPSPointData,
  point2: GPSPointData,
): GPSPointData {
  ...
  if (point1.altitude && point2.altitude) {
    ...
    altitude3 =
      (altitude1 * distance2 + altitude2 * distance1) / (distance1 + distance2);
